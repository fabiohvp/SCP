...
class File {
    validateScpFile(textDocument) {
        let diagnostics = [];
        let issues = 0;
        let lines = textDocument.getText().split(/\r?\n/g);
        for (var i = 0; i < lines.length && issues < settings_1.Settings.scp.maxNumberOfProblems; i++) {
            let line = lines[i];
            variables_1.scp.issueCommands.forEach(function (command) {
